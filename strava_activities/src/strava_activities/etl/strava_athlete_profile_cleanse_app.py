...
def main():
    ...
    cleanse_table_name = config['databricks']['cleanse_table']['cleanse_table_name']
    ...
    cleanse_df = raw_df.selectExpr(
        ...
        "strip(athlete_profile:firstname) as first_name",
        "strip(athlete_profile:lastname) as last_name",
        "cast(athlete_profile:badge_type_id as int) as badge_type_id",
        "strip(athlete_profile:bio) as bio",
        "strip(athlete_profile:city) as city",
        "strip(athlete_profile:state) as state",
        "strip(athlete_profile:country) as country",
        "cast(athlete_profile:premium as boolean) as premium",
        "athlete_profile:profile as profile",
        "athlete_profile:profile_medium as profile_medium",
        "cast(athlete_profile:resource_state as int) as resource_state",
        "athlete_profile:sex as sex",
